...
class FPSmetric:
    ...
    def __init__(
      ...
      self._frame_time = set_start_time
    ...
    def __call__(self, frame:np.ndarray = None) -> typing.Union[bool, np.ndarray]:
       ...
       print("previous_time:",self._prev_frame_time, "current time:", self._frame_time)
       
       
       if not self._prev_frame_time:
          return frame
       self._fps_list.append(1 / (self._frame_time - self._prev_frame_time))
       self._fps_list = self._fps_list[-self._range_average:] ## gets the last 30 elements
       ...
       cv2.putText(frame, str(int(fps))+" fps", self.position, 
                  self.fontFace, self.fontScale, self.color, self.lineType)
       return frame
